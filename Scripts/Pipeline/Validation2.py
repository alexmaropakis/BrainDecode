...
# sample_map = pd.read_excel(scratch_dir + 'Dependencies/sample_map/          ') # edit sample map
...
for s in samples:
    print(s)
    evidence = pd.read_csv(MQ_dir+'evidence.txt', sep='\t', engine='python')
...
for s in samples:
    ...
    for k,v in val_mtp_dict[s]['aa subs'].items():
        ...
        for idx in ev_idx:
            ...
            if len(scan_row)>0:
                matches_val = scan_row['Matches'].values[0]
