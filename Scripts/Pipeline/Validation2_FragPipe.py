...
for s in samples:
    print(s)
    evidence = pd.read_csv(Frag_dir+'psm.tsv', sep='\t', engine='python')
...
frag_tsv_dict = {}
# for f in glob(Frag_dir+'*FR*.tsv') + glob(Frag_dir+'*fraction*.tsv') :
  #  raw = os.path.splitext(os.path.basename(f))[0]
  # frag_tsv_dict[raw] = pd.read_csv(f, sep='\t', low_memory=False)

_SUMMARY = {'psm.tsv', 'ion.tsv', 'peptide.tsv', 'protein.tsv'}
frag_tsv_dict = {}
for f in glob(Frag_dir + '*.tsv'):
    if os.path.basename(f) in _SUMMARY:
        continue
    raw = os.path.splitext(os.path.basename(f))[0]
    frag_tsv_dict[raw] = pd.read_csv(f, sep='\t', low_memory=False)
