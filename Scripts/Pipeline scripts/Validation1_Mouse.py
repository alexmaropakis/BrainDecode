...
# Last updated 05-22-2026 by Alex Maropakis
...
uniprot_fasta_str   = database_dir + 'MOUSE_UP000000589_10090.fasta'

mtp_dict = pickle.load(open(aas_dir+'MTP_dict.p', 'rb'))
samples  = list(mtp_dict.keys())
fasta_str_list = [noMTP_fasta for s in samples]
...
for s in samples:
    ...
    fasta = os.path.join(appended_dir,os.path.basename(ff).replace('_noMTP.fasta', '_MTP.fasta'))
    ...
    with open(fasta, 'w') as f:
        ...
        for i,seq in enumerate(unq_seqs):
            ...
            mtp_id = f"MTP{mtp_global_counter:08d}"
            mtp_global_counter += 1
            f.write('>MTP|'+seq_idx+'_base'+bp_scan+'_DP'+dp_scan+'_'+mtp_id+'\n'+seq+'\n')
        f.close()         
